...
void Initialize_MD()
{
	...
	//DefineMol();				//defines water molecule details
	//InitAngCoords();
	//InitAngVels();
	//InitAngAccels();
...
void GetParameters(char **argv)
{
    ...
    {	
		...
		ffscanf("mass", file, "%lf", &mass);
		ffscanf("nMol", file, "%d", &nMol);
		ffscanf("BondLen", file, "%d", &BondLen);
		ffscanf("ProtLen", file, "%d", &ProtLen);
		ffscanf("nC", file, "%d", &nC);
		ffscanf("LV", file, "%lf", &LV);		
		ffscanf("dt", file, "%lf", &dt);
		ffscanf("rCut", file, "%lf", &rCut);
		ffscanf("cT", file, "%lf", &cT);
		ffscanf("displayInterval", file, "%d", &displayInterval);	
		ffscanf("stepLimit", file, "%d", &stepLimit);
		ffscanf("step2rescVels", file, "%d", &step2rescVels);
		ffscanf("temperature", file, "%lf", &temperature);
		ffscanf("temperature_steps", file, "%lf", &temperature_steps);
		ffscanf("report_file", file, "%c", &report_file);
		ffscanf("step2report", file, "%d",  &step2report);
		ffscanf("printsummary", file, "%c", &printsummary);
		ffscanf("printsummaryInterval", file, "%d", &printsummaryInterval);
		ffscanf("printsummary2file", file, "%c", &printsummary2file); 
		ffscanf("shake-consPrec", file, "%lf", &consPrec);
		ffscanf("shake-maxCycle", file, "%d", &maxCycle);
		ffscanf("shake-step2shake", file, "%d", &step2shake);
...
int Step() 
{
	...
	if (stepCount <= stepLimit)			//function that handles the processing for a single step (or iteration)
	{
		velocityVerlet();
		...
		evaluate(); // pega os resultados mais recentes
	...
	{
		tp = time(NULL) - ti; 	//calculates processing time, ti is defined in main.c

		report();
		//saveBestcoordinates();	//it saves best structure coordinates in a txt file
...
void report()
{
	...
	if (bondavg <= 1) //bondavg > 1 --> protein passes across box's boundary
...
// coleta o tamanho de moléculas e o tempo de execussão do programa
void time_ex()
{	
	FILE *arqv;	
	arqv = fopen("time.txt", "a");				// vamos só abrir o arquivo e colocar mais dados nele
												// "a" permissão para abrir e escrever, se não existir o arquivo, cria
...
//
	...
	for (i = 0; i < ProtLen -3; i++)
	{
		...
		f = 0.5; //* sqrt(c11) * sqrt(c33);  // -du(theta)/d(cos (theta))  = k2 * abs(bi) * abs(b_(i+1)) * 1 ); component
		...
		mol[i].ra.x = mol[i].ra.x + fi1.x;														//mol[i] : (i-2)-atom  -- f(i-2)
...
//debug-cesar:
//printf("cosbeta (torsion) = %lf\n", c13);


//debug-cesar
/*printf("Four atoms forces (modules) : \n ");
printf("Force (i-2)-atom = %lf \n", sqrt(fi1.x * fi1.x + fi1.y * fi1.y + fi1.z * fi1.z));
printf("Force (i-1)-atom = %lf \n", sqrt(((- (1. + cr1) * fi1.x) + cr2 * fi2.x ) * ((- (1. + cr1) * fi1.x) + cr2 * fi2.x ) + ((- (1. + cr1) * fi1.y) + cr2 * fi2.y) *((- (1. + cr1) * fi1.y) + cr2 * fi2.y) + ((- (1. + cr1) * fi1.z) + cr2 * fi2.z) *  ((- (1. + cr1) * fi1.z) + cr2 * fi2.z) ) );

printf("Force (i)-atom =  %lf\n", sqrt((cr1 * fi1.x + (- (1. + cr2)) * fi2.x)*(cr1 * fi1.x + (- (1. + cr2)) * fi2.x)+(cr1 * fi1.y + (- (1. + cr2)) * fi2.y)*(cr1 * fi1.y + (- (1. + cr2)) * fi2.y) + (cr1 * fi1.z + (- (1. + cr2)) * fi2.z)*(cr1 * fi1.z + (- (1. + cr2)) * fi2.z)));

printf("Force (i+1)-atom =  %lf\n", sqrt(fi2.x*fi2.x + fi2.y*fi2.y + fi2.z*fi2.z));

sum_force.x = fi1.x + fi2.x + (- (1. + cr1) * fi1.x) + cr2 * fi2.x +  cr1 * fi1.x + (- (1. + cr2)) * fi2.x; 
sum_force.y = fi1.y + fi2.y + (- (1. + cr1) * fi1.y) + cr2 * fi2.y +  cr1 * fi1.y + (- (1. + cr2)) * fi2.y;
sum_force.z = fi1.z + fi2.z + (- (1. + cr1) * fi1.z) + cr2 * fi2.z +  cr1 * fi1.z + (- (1. + cr2)) * fi2.z;

printf("sum_Forces = %lf\n", sqrt(sum_force.x*sum_force.x + sum_force.y*sum_force.y +sum_force.z*sum_force.z));  //sum(forces) = 0


printf("mol[i].ra.x = %lf\nmol[i + 1].ra.x = %lf\nmol[i + 2].ra.x = %lf\nmol[i + 3].ra.x = %lf\n", mol[i].ra.x, mol[i + 1].ra.x, mol[i + 2].ra.x, mol[i + 3].ra.x);//getchar();
//
*/
...
void rescaleVelocities() 
{
	...
	velMag = sqrt(NDIM * (1. - 1. / nMol) * temperature);  //update velMag 
														   //(the temperature can be modified by clicking the temperature++ or temperature-- buttons)
	

	//Temperatura adjustment --> translational motion
    for (i = 0; i < nMol; i++)
	{
		sum += mol[i].rv.x * mol[i].rv.x + mol[i].rv.y * mol[i].rv.y + mol[i].rv.z * mol[i].rv.z;
	}
	...
	//Temperatura adjustment --> rotational motion
	sum = 0;
    for (i = 0; i < nMol; i++)
	{
...
void calc_AngVelocities(int n, VectorR *w)
{
	...
	//qt.u4 = -qvt.u1 * mol[n].q.u1 - qvt.u2 * mol[n].q.u2 - qvt.u3 * mol[n].q.u3 + qvt.u4 * mol[n].q.u4; //not needed
	
	//scale
	qt.u1 = 2 * qt.u1;
	qt.u2 = 2 * qt.u2;
	qt.u3 = 2 * qt.u3;
	//qt.u4 = 2 * qt.u4;	/not needed
...
void calc_AccelQuat()
{
    ...
    for (n = 0; n < nMol; n++)
	{
		calc_AngVelocities(n, &w);			//paralelizar isto
...
void evaluate()							// calcula as melhores características encontradas
{
	int n;
...
void savePathways()
{
	/*int i;
	FILE *coordFile;
	coordFile = fopen("pathways.txt", "a");
	
	fprintf(coordFile, "N\tx\ty\tz\n");
	for (i=0;i<ProtLen;i++) // i é o aminoácido
	{
		fprintf(coordFile, "%d\t%lf\t%lf\t%lf\n", i, mol[i].r.x, mol[i].r.y, mol[i].r.z);
	}
	fprintf(coordFile, "\n\n");
...
void savePathwaysVetor(int pos) // salva cada estrutura no vetor de conformations, ainda não salva no arquivo
{
	int i;
...
void savePathwaysVetorArquivo() // Passa para o arquivo
{
	int i, k;
	FILE *coordFile;
