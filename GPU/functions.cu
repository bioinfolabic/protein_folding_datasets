// Versão 17 - todo o step dentro da GPU.
...
int loadFile(char **argv) {
	...
	if(fi != 0) {
		...
		getParameter("mass", "=", "%lf", &mass, fi);
		getParameter("nMol", "=", "%d", &n_mol, fi);
		getParameter("BondLen", "=", "%d", &bond_len, fi);
		getParameter("ProtLen", "=", "%d", &prot_len, fi);
		getParameter("nC", "=", "%d", &n_c, fi);
		...
		getParameter("step2rescVels", "=", "%d", &step2resc_vels, fi);
		getParameter("temperature", "=", "%lf", &temperature, fi);
		getParameter("temperature_steps", "=", "%lf", &temperature_steps, fi);
		getParameter("report_file", "=", "%c", &report_file, fi);
		getParameter("step2report", "=", "%d", &step2report, fi);
		getParameter("printsummary", "=", "%c", &print_summary, fi);
		getParameter("printsummaryInterval", "=", "%d", &print_summary_interval, fi);
		getParameter("printsummary2file", "=", "%c", &print_summary2file, fi);
		getParameter("shake-consPrec", "=", "%lf", &shake_cons_prec, fi);
		getParameter("shake-maxCycle", "=", "%d", &shake_max_cycle, fi);
		getParameter("shake-step2shake", "=", "%d", &shake_step2shake, fi);
...
double randdouble(double max) {
	...
	//printf("Comparacao\ndouble=%13.9f \n Inteiro: %d \n Intervalo: %lf\n", fabs(randomMT()), randomMT(), fabs(((randomMT() + (RAND_MAX_MT/2)) / ((double)(RAND_MAX_MT)) * max))); // + (RAND_MAX_MT/2)) / ((double)(RAND_MAX_MT)) * max));

	//return fabs(((randomMT() + (RAND_MAX_MT/2)) / ((double)(RAND_MAX_MT)) * max));
...
__global__ void cudaBondEnergy(Particle *particles, double *d_uB, double LV) {
    // Por enquanto um bloco so. Limite de 1024 threads/aminoacidos (compute capability >= 2.x)
    ...
    if(threadId < (d_nmol - 2)) {
        ...
        cudaVerifyBoundary2(&dr1, LV);  // Para o caso de uma particula estar na borda da caixa e a seguinte, do outro lado
    ...
    // Somando a energia por reducao (Optimizing Parallel Reduction in CUDA, Mark Harris)
    if (blockSize >= 512) {
        if (threadId < 256)
...
__global__ void cudaTorsionEnergy(Particle *particles, double *d_uT, double LV) {
    ...
    if(threadId < (d_nmol - 3)) {
        ...
        //particles[threadId + 3].v_a.x = 0;
        //particles[threadId + 3].v_a.y = 0;
        //particles[threadId + 3].v_a.z = 0;
        //__syncthreads();
        particles[threadId].v_a.x += a1.x;
        particles[threadId].v_a.y += a1.y;
        particles[threadId].v_a.z += a1.z;
...
/*__global__ void cudaZeroOut(Particle *particles) {
	int threadId = threadIdx.x;
...
__global__ void cudaLJcomp(Particle *particles, char *d_sequence, double LV, double *d_uLJVector, int i, int n_mol) {
	...
	//d_uLJComp[j] = 1.;
	//printf("j: %d - d_uLJComp[j]: %.8lf\n", j, d_uLJComp[j]);
    // ============================================================================================================================ //
	// Parallel reduction
	// (Optimizing Parallel Reduction in CUDA, Mark Harris)
    __syncthreads(); // espera rodar para todos os valores de j
    //printf("%d - %lf", j, d_uLJComp[j]);
	...
	if (blockSize >= 8){
		if(j < 4){
			d_uLJComp[j] += d_uLJComp[j + 4];} //printf("\n%d - %lf", j, d_uLJComp[j]);}
	}
	__syncthreads();
	if (blockSize >= 4){
		if(j < 2){
	...
	if (blockSize >= 1){
		...
		//printf("FIM!!! -> i: %d   d_uLJVector[%d]: %.8lf\n", i, i, d_uLJVector[i]); //print 2
	}

	//////////////////////////////////////////////////////////////////////////////////////////////////////////////
    /*if (blockSize >= 1024) {
        if (j < 512)
            d_uLJVector[i * blockSize + j] += sd_uLJVector[i * blockSize + j + 512];
        __syncthreads();
    }
    if (blockSize >= 512) {
        if (j < 256)
            d_uLJVector[i * blockSize + j] += d_uLJVector[i * blockSize + j + 256];
        __syncthreads();
    }
    if (blockSize >= 256) {
        if (j < 128)
            d_uLJVector[i * blockSize + j] += d_uLJVector[i * blockSize + j + 128];
        __syncthreads();
    }
    if (blockSize >= 128) {
        if (j < 64)
            d_uLJVector[i * blockSize + j] += d_uLJVector[i * blockSize + j + 64];
        __syncthreads();
    }

    if (j < 32) {
        if (blockSize >= 64) d_uLJVector[i * blockSize + j] += d_uLJVector[i * blockSize + j + 32];
        __syncthreads();
        if (blockSize >= 32) d_uLJVector[i * blockSize + j] += d_uLJVector[i * blockSize + j + 16];
        __syncthreads();
        if (blockSize >= 16) d_uLJVector[i * blockSize + j] += d_uLJVector[i * blockSize + j + 8];
        __syncthreads();
        if (blockSize >= 8) d_uLJVector[i * blockSize + j] += d_uLJVector[i * blockSize + j + 4];
        __syncthreads();
        if (blockSize >= 4) d_uLJVector[i * blockSize + j] += d_uLJVector[i * blockSize + j + 2];
        __syncthreads();
        if (blockSize >= 2) d_uLJVector[i * blockSize + j] += d_uLJVector[i * blockSize + j + 1];
    }*/
...
__global__ void cudaLJ(Particle *particles, char *d_sequence, double LV, double *d_uT, double *d_uB, double *d_uLJ, double *d_uLJVector, int step, int n_mol){
	...
	unsigned int blockId = blockIdx.x;; /*= blockIdx.x;*/ /*Só queremos calcular LJ e não as demais energias*/
...
__global__ void cudaEnergy(Particle *particles, char *d_sequence, double LV, double *d_uT, double *d_uB, double *d_uLJ, double *d_uLJVector, int step) {
	...
	if(blockId == 1) {
		if(threadId < (d_nmol - 3)) {
			...
			// Atomic add for doubles (CC 6.0 or higher) - Kerana - Titan Xp
			atomicAdd(&particles[threadId].v_a.x, a1.x);
			atomicAdd(&particles[threadId].v_a.y, a1.y);
			atomicAdd(&particles[threadId].v_a.z, a1.z);
...
__global__ void cudaShakeRelaxation(Particle *particles, Constraint *constraint, int n_c, double shake_cons_prec, double LV) {
	...
	//printf("Dentro do shake - %d", i_step);
...
void report(char **argv) {
    ...
    sprintf(file_name, "%s_report_file_%s.txt", argv[2], argv[4]);

	fo = fopen(file_name, "a+");

	if (i_step == 0)
		fprintf(fo, "Step\tTime\tCurrent Temperature\tKinetic Energy\tPotential Energy\tInternal Energy\tDensity\trGH\trGP\trGAll\tBond Length Average\n");

	//if (bond_avg <= 1)
	fprintf(fo, "%d\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\n", i_step, i_step * dt, current_temperature, kinetic_energy, uSum,  total_energy, density, rGH, rGP, rG, bond_avg);
...
void savePathways(char **argv) {
    ...
    sprintf(file_name, "%s_%s.txt", argv[2], argv[4]);
	fo = fopen(file_name, "a+");
...
void saveBestCoordinates(char **argv) {
    ...
    sprintf(file_name, "%s_bestCoordinates_%s.txt", argv[2], argv[4]);
	fo = fopen(file_name, "w");
	for(i = 0; i < n_mol; i++) {
		fprintf(fo, "%d\t%lf\t%lf\t%lf\n", i, best_structure[i].v_r.x, best_structure[i].v_r.y, best_structure[i].v_r.z);
	}

	fprintf(fo, "BestEnergy = %lf\nStep = %d\n", best_potencial_energy, best_step);
	fprintf(fo, "rGAll = %lf\nrGH = %lf\nrGP = %lf\n", rG, rGH, rGP);
	fprintf(fo, "Processing Time = %lfs\n", (double) delta_t);	//processing u
	fclose(fo);*/
...
void finishSim(char **argv) {
	finTimer();
	report(argv);  /// Descomentar depois de arrumado
	savePathways(argv);
	saveBestCoordinates(argv);
	freeArrays();
    freeDevice();
}
